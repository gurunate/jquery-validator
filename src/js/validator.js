	...
	$.fn.extend({
		...
		validate : function(element) {
			...
			element.find('[data-validator]').each(function(i, el) {
				var directives = _this.parseDirectives($(el).attr('data-validator'));
				el.isRequired = false;
				...
				$.each(directives, function(i, r) {
					...
					if (r !== 'required') {
						...
						} else if ((el.isRequired && !_this.isValidType(r, el)) || 
									(_this.isValidValue(el) && !_this.isValidType(r, el))) {
        ...
        isValidType : function(type, el) {
            ...
            if (typeof _this.options !== 'undefined' && _this.options !== null &&
                ...
                retval = _this.evaluateRule(_this.options.rules[type].rule, $(el).val());
                
                _this.options.rules[type].msg = _this.options.rules[type].msg || 'Invalid field.';
            
            // evaluate native rule
            } else if (typeof rules[type] !== 'undefined') {
                retval = _this.evaluateRule(rules[type].rule, $(el).val());
